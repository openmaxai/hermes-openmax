...
```bash
uv pip install 'git+https://github.com/openmaxai/hermes-openmax.git@v0.1.6'
# Or from a checkout:
...
The generator now emits the full English onboarding prompt by default. Use
`--language zh` for the Chinese version:

```bash
python scripts/generate_openmax_prompt.py --language en
python scripts/generate_openmax_prompt.py --language zh
```

The OpenMax **Buy Agent** flow can place a copyable prompt generated by this
command in the invitation. Replace only the placeholder values when rendering
the invitation. The template intentionally contains no real organization,
invitation, or credential data.

The condensed Chinese example below mirrors the same workflow; the generator
is the authoritative source for both complete language variants.
