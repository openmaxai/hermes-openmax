...
| 建项目 | 创建项目;用户必须明确要求创建,不得把“找不到项目”自动解释成新建 | `workspace_tasks(action=project_create, body={name, slug?, lead_member_id, description?, knowledge_base_id?, member_ids?})` |
| 查项目 | 获取单项目详情 | `workspace_tasks(action=project_get, project_id)` |
| 更新项目 | 修改项目元数据 | `workspace_tasks(action=project_update, project_id, body={name?, description?, lead_member_id?})` |
| 归档 | 前端“删除”是可恢复归档,不是硬删;当前原生工具不暴露恢复动作 | `workspace_tasks(action=project_archive, project_id)` |
| 项目成员 | 列出、添加、移除项目成员 | `workspace_tasks(action=project_members, project_id)` / `workspace_tasks(action=project_member_add, project_id, member_id)` / `workspace_tasks(action=project_member_remove, project_id, member_id)` |
...
改 Owner / 移动项目走同一语义动作入口:
`workspace_tasks(action=issue_action, issue_id, name="reassign-owner",
body={new_owner_member_id})`、`workspace_tasks(action=issue_action, issue_id,
name="move", body={new_project_id})`。执行前先重读 Issue 并确认目标,这两项都是高代价治理动作。
...
查单任务:`workspace_tasks(action=task_get, task_id)`。
...
| 全量改步骤 | 一次性替换全部 Step,**不是追加** | `workspace_tasks(action=blueprint_set_steps, blueprint_id, body={steps:[...]})` |

当前没有单 Step 增删改;需要调整时用 `blueprint_set_steps` 提交**完整的新 steps 数组**。
计划确认走 `issue_action(submit-plan)`,即
`issue_action(name="submit-plan", body={blueprint_id, plan_text})`;当前工具没有独立
`blueprint_submit` 动作。
...
查单个定时任务:`workspace_tasks(action=binding_get, binding_id)`。
...
3) 需要改 Steps 时用 blueprint_set_steps 全量替换,不是追加
