...
| 建 KB | 新建 KB 实例 | `workspace_kb(action=kb_create, body={name, visibility?, description?, icon?})` |

**当前工具未暴露,如需要请告知 owner**:kb.init、kb.get、kb.update、kb.delete、kb.archive / kb.unarchive。
...
| 登记文件节点 | 把已有 artifact 登记进树 | `workspace_kb(action=create_file, kb_id, name, artifact_id, parent_id?)` |
| 批量下载 | 批量获取下载信息 | `workspace_kb(action=batch_download, kb_id, node_ids=[...])` |

**当前工具未暴露,如需要请告知 owner**:kb.node_get、kb.node_breadcrumb、kb.node_children、kb.file_preview。
...
| 改元数据 | 修改标题/path 等 | `workspace_kb(action=page_update, page_id, body={title?, path?})` |
| 永久删除 | 永久删除页面;危险,通常先 trash 并确认 | `workspace_kb(action=page_delete, page_id)` |
| 冻结 | 页面设为只读 | `workspace_kb(action=freeze, page_id)` |
| 查引用 | 列页面引用位置 | `workspace_kb(action=references, page_id)` |
...
**当前工具未暴露,如需要请告知 owner**:kb.pages、kb.page_revision。
...
**三态保护链:trash → page_delete(永久删除)**:

- 永久删除(物理删除)要求页面已处于 `trashed` 态:必须先 `trash` 扔进回收站,
  向 owner 明确说明不可恢复并获得确认后,再调用 `page_delete`。
- 对 `active` 页面直接永久删除,服务端返回 404(语义保护,不要绕过)。
- 完整链:`create_page → ... → trash → 人类确认 → page_delete`。中途反悔用
  `restore` 拉回;**再删时必须重新 trash 一次**。
...
```
{前端基址}/workspace/knowledge?kb={kb_id}&node={node_id}
                                          ↑ 必须是树节点 node_id,不是 page_id
```

**反例(错误)**:`...&node=pg-abc123` —— 用了页面 id,链接能打开但空白或指向错误位置。

用 `workspace_members(action=frontend_url, path="knowledge?kb=...&node=...")` 生成完整链接,不要手拼域名。
...
- 永久删除必须先 `trash`(三态保护链,直接删返回 404,不要绕过);确认后用 `page_delete`
