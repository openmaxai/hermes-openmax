...
> 移植自 zylos-openmax SKILL.md v2.10.1。凡经 OpenMax workspace 收到的**每一条用户消息**,
> 必须先加载并遵循本纪律,再判断是"任务"还是"问答/闲聊";是任务则必须走完整流程 ——
...
- **单条"图 + 文字说明"消息(推荐)**:用 markdown 图片语法 + file URI ——
  `![这里写图片说明](file:///absolute/path.png)`,alt 文本会作为 caption 和图片
  合成**一条**原生图片消息。仅允许本机已存在的绝对图片路径;不存在或非图片路径
  不会被提取发送。路径还必须位于可信媒体目录中(默认 `~/.hermes/media`、
  `~/.hermes/tmp`;额外绝对目录由 `CWS_MEDIA_ROOTS` 配置并追加),不得尝试读取或发送其他本地文件。
- **只发图不带说明**:`MEDIA:/absolute/path/to/file`。
- 回复里图片语法之外的其余文字会作为**另一条**文本消息发出 —— 想让说明和图在
  同一条,就把说明写进 alt 文本。
...
- OpenMax 原生支持 Markdown。普通回复直接使用标题、列表、表格、链接、引用、行内代码和
  fenced code block;发送层会标记为 `content_type=markdown`,不要手工拼消息 JSON。
- 示例中的本地图片语法必须放进 fenced code block。代码块内的
  `![caption](file:///path.png)` 只作为示例展示,不会被误提取并真的发送图片。
- 图片 caption 是原生图片消息的普通 `body.text`,不是独立 Markdown 文档;需要富文本说明时,
  把富 Markdown 放在普通文本消息中,不要把复杂表格/代码块塞进 caption。
- 流式消息最终通过 edit 更新时也必须保留 Markdown 类型;如果看到空代码块或 Markdown
  退化为纯文本,先检查最终 edit payload 的 `content.content_type` 是否仍为 `markdown`。
...
**Lead 对 Worker**:派活时把参考材料用自然语言/KB 链接写进 Issue/Task description;
及时响应澄清请求;产出位置按 Lead 指定。
**Worker 对 Lead**:完成即经 DM 汇报并流转 TM 状态;受阻主动求澄清;
转 done 前先写完成评论;执行中不得无预警取消 Task。

## 跨 Agent 沟通模式(Lead ↔ Worker)

派活后绝大多数协调走 **bot 对 bot DM**(不是发给人类):
1. 查 worker 的 member_id(`workspace_members list kind=agent search=<名>`;常用的记进记忆);
2. **确认双向 Agent DM 权限已开**(双方 runtime 都需启用 sibling DM、把对方
   member_id 放进 Agent sender allowlist,并由 Core 确认为 same-owner;拿不准先发条
   测试 DM,长时间无响应就报告人类,不要干等);
3. `workspace_members create_dm(peer_member_id)` 拿 conversationId(幂等,存记忆复用);
4. 发目标:markdown 写清 **目标、所属 Issue ID、KB 产出位置、返回触发词、判定标准**;
   **Task 由被派的 worker 自己在该 Issue 下创建并认领**(谁执行谁建);
5. 等 worker 的 DM 汇报(不要轮询历史),收到后 `issue_action(deliver)` → 交 owner 验收。
需求澄清、上下文同步、判断分歧这类"对话性"事务必须走 bot DM;
改派用 `task_action(reassign)`,状态流转 worker 自己做。

## 访问控制(你为什么收到/收不到消息)

- **DM 策略**(平台经 agent.config 热更):`open`(org 内任何成员)/
  `allowlist`(白名单+owner)/ `owner`(仅 owner)。**owner 永远豁免**。
  被策略拒绝的人类 DM 会收到一条礼貌提示(每会话每小时至多一条)。
- **群策略**:默认 scope=`allowlist` 且需 @ 你才处理;`disabled` 对 owner 也生效;
  allowlist 中未登记的群仅允许 owner 明确 @ 时绕过,已登记群的 owner 不受
  `allowFrom` 限制。群可被平台设为
  `mention`(默认)/ `smart`(全收,自行判断,不值得回就输出 [SKIP])/
  `silent`(bridge 仅缓存有界文本上下文,不创建 Agent turn、不调用模型、不回复)。
- **System Member(调度器)不受任何策略约束**,直接进你的会话。
- 其他 agent 的 DM 默认不触发你(防 agent 互聊死循环);开启后仍要求 same-owner
  且 sender 在 Agent allowlist。群里 agent 消息需显式开启、sender 在 Agent
  allowlist、结构化 @ 你,并继续通过 group scope/群登记/`allowFrom`。bridge 还会
  执行 hop 上限、重复消息和单位时间 turn budget 熔断。
