...
def _cfg():
    return CwsConfig(
        bff_url="https://bff.test", ws_url="wss://comm.test",
        api_key="cwsk_x", org_id="org-1", member_id="me-1",
    )
...
def _http_for(handler):
    ...
    def h(request):
        if request.url.path == "/auth/agent/token":
            return httpx.Response(200, json={"data": {"access_token": "jwt", "refresh_token": "rt"}})
        return handler(request)

    tm = TokenManager(cfg, http=httpx.AsyncClient(transport=httpx.MockTransport(h)))
    return CwsHttpClient(cfg, tm, http=httpx.AsyncClient(transport=httpx.MockTransport(h)))
...
async def test_online_report():
    ...
    def handler(request):
        ...
        return httpx.Response(200, json={"data": {"triggered": False, "reason": "already_onboarded"}})
...
async def test_metrics_report_version_only():
    ...
    def handler(request):
        if request.url.path.endswith("/runtime-metrics"):
            ...
            return httpx.Response(200, json={"data": {"request_id": "r", "server_time": "t"}})
...
async def test_billing_gate_suspended_and_cache():
    ...
    def handler(request):
        ...
        return httpx.Response(200, json={"data": {"usage_snapshot": {"enforcement_suspended": True}}})
...
async def test_config_hot_update_frames(tmp_path):
    ...
    b = CwsBridge(
        _cfg(), storage=FileStorage(tmp_path), on_message=on_message,
        billing_gate_enabled=False, on_config_event=on_cfg,
    )

    def sysframe(event, data):
        return Frame(type=FRAME_SYSTEM, payload={"event": event, "data": data})

    await b._handle_frame(sysframe("agent.config.dm_allowlist_changed",
    ...
    await b._handle_frame(sysframe("agent.config.group_mode_changed",
    ...
    await b._handle_frame(sysframe("agent.config.owner_changed",
                                   {"new_owner_member_id": "owner-2"}))
